...
```{r}
data <- read.csv("~/Desktop/MsCcSurvJun9.csv", header = TRUE)
```
...
```{r}
ggplot(aes(x = day.mass.pupa, y = mass.pupa, color = food), data = data[data$para == "no", ]) + 
  geom_jitter(alpha = 0.8, width = 0.2) +  
  scale_color_manual(values=c("#95D840", "#59CCC3", "#3F87BC"),
                     name="Food Type",
                     label=c("Devil's Claw", "Diet", "Tobacco"))
  
```

No obvious relationship - I don't think we need to account for it in our model. 
...
No interaction of diet overall
...
ggsave("MsCcSurvJun9.png", width = 8, height = 5)
...
ggsave("MsCcSurvParaJun9.png", width = 8, height = 5)
...
ggsave("MsCcSurvRawJun9.png", width = 8, height = 5)
...
```{r}
we2 <- lmer(num.w.tot ~ food + num.para + (1|bin), data = data[data$para == "yes", ])
...
### Percent wasps that were cocoons

```{r}
per1 <- lmer(perc.w.coc ~ food + (1|bin), data = data[data$para == "yes", ])

anova(per1)
```

```{r}
per2 <- lmer(perc.w.coc ~ food + num.para + (1|bin), data = data[data$para == "yes", ])

anova(per2)
```

No effect of diet on percent that were cocoons, but strong effect of number of parasitizations
