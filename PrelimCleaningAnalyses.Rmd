...
```{r}
ml1 <- lmer(mass.lost.pupa ~ food + (1|bin), data = data)
...
```{r}
per2 <- lmer(perc.w.coc ~ food + num.para + food:num.para + (1|bin), data = data[data$para == "yes", ])

anova(per2)
```

No effect of diet on percent that were cocoons, but strong effect of number of parasitizations (goes away when you add the interaction term). 
...
```{r}
per3 <- lmer(perc.w.coc ~ food + num.para + food:num.para + (1|bin), 
            data = data[data$para == "yes" & data$num.para != 2, ])

anova(per3)
```

Now there's a significant effect of food and number of parasitizations. 
